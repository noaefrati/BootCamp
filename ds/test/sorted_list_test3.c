/*****************************************************************************
* Description: Program to test Sorted linked list implementation
* Author: Noa Efrati
* Reviewer:
*
* ILRD - OL97
...
#include "sorted_list.h"      	/* dlist core files                         */
...
static void TestForNextAndPrev(void)
{
	...
	/*
	TEST("TestForNextAndPrev", SRTLSTIsSameIter((iter = SRTLSTPrev(SRTLSTPrev(lst))), SRTLSTPrev(lst)), 1);

	TEST("TestForNextAndPrev",SRTLSTIsSameIter(SRTLSTNext(iter), SRTLSTEnd(lst)), 1);
	SRTLSTInsert(lst, data2);
	
	TEST("TestForNextAndPrev",SRTLSTIsSameIter((iter2 = SRTLSTBegin(lst)), STRLSTPrev(iter)), 1);
	TEST("TestForNextAndPrev", SRTLSTIsSameIter(STRLSTNext(iter2), iter), 1);
	TEST("TestForNextAndPrev", SRTLSTIsSameIter(STRLSTNext(STRLSTNext(iter2)), SRTLSTEnd(lst), 1));
	*/
	SRTLSTDestroy(lst);	
}
